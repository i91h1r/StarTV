...
import com.example.hyr.startv.utils.BitmapUtils;

/**
 * Description:
 * 作者：hyr on 2016/11/23 10:04
 * 邮箱：2045446584@qq.com
 */
public class FloatingVideoMeun extends ViewGroup {
    private static final int SHADOW_OFFSET = 20;

    private FloatingVideoButton floatingVideoButton;
    ...
    public FloatingVideoMeun(Context context) {
        this(context, null);
    }

    public FloatingVideoMeun(Context context, AttributeSet attrs) {
        super(context, attrs);
        initMenu(context, attrs);
    }

    public FloatingVideoMeun(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initMenu(context, attrs);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public FloatingVideoMeun(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void initMenu(Context context, AttributeSet attrs) {
        ...
        TypedArray attr = context.obtainStyledAttributes(attrs, R.styleable.FloatingVideoMeun, 0, 0);
        progressWidthPercent = attr.getInteger(R.styleable.FloatingVideoMeun_fvm_progress_percent, 3);
        progressColor = attr.getColor(R.styleable.FloatingVideoMeun_fvm_progress_color, getResources().getColor(android.R.color.holo_blue_dark));
        progress = attr.getFloat(R.styleable.FloatingVideoMeun_fvm_progress, 0);
        buttonSpace = attr.getDimension(R.styleable.FloatingVideoMeun_fvm_botton_space, 4);
        buttonSpace = dp2px(buttonSpace);
        if (Build.VERSION.SDK_INT < 21) {
            // 版本兼容
            buttonSpace = -BitmapUtils.dp2px(16);
        }
        cover = attr.getDrawable(R.styleable.FloatingVideoMeun_fvm_cover);
        backgroundTint = attr.getColorStateList(R.styleable.FloatingVideoMeun_fvm_backgroundTint);
        attr.recycle();
    ...
    private void createRootButton(Context context) {
        floatingVideoButton = new FloatingVideoButton(context);
        floatingVideoButton.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                toggle();
            }
        });
        floatingVideoButton.config(progressWidthPercent, progressColor, backgroundTint);
        floatingVideoButton.setProgress(progress);
        if (cover != null) {
            floatingVideoButton.setCoverDrawable(cover);
        }
    }

    public void setProgress(float progress) {
        if (floatingVideoButton != null) {
            floatingVideoButton.setProgress(progress);
        }
    }
    ...
    protected void onFinishInflate() {
        ...
        addView(floatingVideoButton, super.generateDefaultLayoutParams());
    }
    ...
    public void setVideoCover(Drawable drawable) {
        floatingVideoButton.setCoverDrawable(drawable);
        if (isRotated) {
            rotateStart();
        } else {
            rotateStop();
    ...
    public void setVideoCover(RotatingProgressDrawable drawable) {
        floatingVideoButton.setCoverDrawable(drawable);
        if (isRotated) {
            rotateStart();
        } else {
    ...
    public void setVideoCover(Bitmap bitmap) {
        floatingVideoButton.setCover(bitmap);
        if (isRotated) {
            rotateStart();
        } else {
            rotateStop();
    ...
    public void rotateStart() {
        isRotated = true;
        floatingVideoButton.start();
    }

    public void rotateStop() {
        isRotated = false;
        floatingVideoButton.stop();
    }
    ...
    public static class Behavior extends CoordinatorLayout.Behavior<FloatingVideoMeun> {
        ...
        @Override
        public void onNestedScroll(CoordinatorLayout coordinatorLayout, FloatingVideoMeun child, View target, int dxConsumed, int dyConsumed, int dxUnconsumed, int dyUnconsumed) {
        ...
        @Override
        public boolean onStartNestedScroll(CoordinatorLayout coordinatorLayout, FloatingVideoMeun child, View directTargetChild, View target, int nestedScrollAxes) {
